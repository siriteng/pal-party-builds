import { cookie, getSessionUser, LIKE_INTENT_COOKIE, parseCookieHeader, redirect, safeReturnTo } from "@/lib/auth";
...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (intent !== id) return redirect(new URL(withBasePath(returnTo), url.origin));
    const user = await getSessionUser(request);
    if (!user) {
      const resume = `${url.pathname}?return_to=${encodeURIComponent(returnTo)}`;
      return redirect(new URL(withBasePath(`/auth/discord?return_to=${encodeURIComponent(resume)}`), url.origin));
    }
    ...
    return redirect(new URL(withBasePath(returnTo), url.origin));
  }
}
