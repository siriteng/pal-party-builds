...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (intent !== id) return Response.redirect(new URL(withBasePath(returnTo), url.origin));
    const user = await getSessionUser(request);
    if (!user) {
      const resume = `${url.pathname}?return_to=${encodeURIComponent(returnTo)}`;
      return Response.redirect(new URL(withBasePath(`/auth/discord?return_to=${encodeURIComponent(resume)}`), url.origin));
    }
    ...
    return Response.redirect(new URL(withBasePath(returnTo), url.origin));
  }
}
