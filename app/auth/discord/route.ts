import { cookie, getSecret, OAUTH_RETURN_COOKIE, OAUTH_STATE_COOKIE, randomToken, redirect, safeReturnTo } from "@/lib/auth";
import { withBasePath } from "@/lib/paths";

export async function GET(request: Request) {
  const url = new URL(request.url);
  const clientId = getSecret("DISCORD_CLIENT_ID");
  if (!clientId) return redirect(new URL(withBasePath("/auth/error?reason=not-configured"), url.origin));
  ...
  const response = redirect(discordUrl);
